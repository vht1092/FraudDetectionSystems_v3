...
public class TimeConverter {
	 ...
	 * @return String
	 */
	public String convertDatetimeYMDHMMSS(final Date date, final boolean ismax) {
		if (date != null) {
			final Calendar cal = Calendar.getInstance();
			cal.setTime(date);
			final int iYyear = cal.get(Calendar.YEAR);
			...
			final int iDate = cal.get(Calendar.DATE);
			if (!ismax) {
				return String.format("%02d%02d%02d", iYyear, iMonth, iDate) + "000000";
			} else {
				return String.format("%02d%02d%02d", iYyear, iMonth, iDate) + "999999";
			}
		}
		return "";
	}
	...
	/**
	 * Convert Date => <code>yyyyMMddHH24MI</code> hoac
	 * <code>yyyyMMddHH24MI</code> cho
	 * muc dich tim kiem. <br>
	 * ismax = true se return chuoi <code>yyyyMMddHH24MI</code> nguoc
	 * lai <code>yyyyMMddHH24MI</code>
	 * 
	 * @param date
	 *            Date
	 * @param ismax
	 ...
	 * @return String
	 */
	public String convertDatetimeYMD(final Date date, final boolean ismax) {
		if (date != null) {
			final Calendar cal = Calendar.getInstance();
			cal.setTime(date);
			final int iYyear = cal.get(Calendar.YEAR);
	...
	//yyyy/MM/dd HH:mm:ss
	public static String convertTimeByFormat(String formatDate) {
