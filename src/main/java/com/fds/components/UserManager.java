...
public class UserManager extends CustomComponent {
	...
	private BeanItemContainer<FdsEbankSysUser> beanSysUserContainer;
	private BeanItemContainer<FdsEbankSysRole> beanSysRoleContainer;
	...
	private BeanFieldGroup<FdsEbankSysUser> formFieldGroup;
	private FdsEbankSysUser selectedUser;
	...
	public UserManager() {
		...
		beanSysUserContainer = new BeanItemContainer<FdsEbankSysUser>(FdsEbankSysUser.class, sysUserService.findAllUser());
		beanSysRoleContainer = new BeanItemContainer<FdsEbankSysRole>(FdsEbankSysRole.class, sysRoleService.findAll());
		// Grid role
		gridRole = new Grid();
		...
		// Edit form
		formFieldGroup = new BeanFieldGroup<FdsEbankSysUser>(FdsEbankSysUser.class);
		...
		return new SelectionListener() {
			...
			public void select(SelectionEvent event) {
				if (gridUser.getSelectedRow() != null) {
					...
					selectedUser = (FdsEbankSysUser) gridUser.getSelectedRow();
					showEditForm(selectedUser);
					List<FdsEbankSysUserrole> userRole = sysUserroleService.findAllByUserId(selectedUser.getUserid());
					List<FdsEbankSysRole> listRole = beanSysRoleContainer.getItemIds();
					MultiSelectionModel multiselectionModel = (MultiSelectionModel) gridRole.getSelectionModel();
					List<FdsEbankSysRole> temp = new ArrayList<FdsEbankSysRole>();
					for (FdsEbankSysRole a : listRole) {
						for (FdsEbankSysUserrole b : userRole) {
		...
		return new CommitHandler() {
			...
			public void postCommit(CommitEvent commitEvent) throws CommitException {
				...
				for (Object a : selectedRole) {
					BeanItem<FdsEbankSysRole> beanScreenItem = beanSysRoleContainer.getItem(a);
	...
	private void showEditForm(FdsEbankSysUser fdsSysUser) {
		if (fdsSysUser == null) {
			fdsSysUser = new FdsEbankSysUser();
		}
		BeanItem<FdsEbankSysUser> beanFdsSysUser = new BeanItem<FdsEbankSysUser>(fdsSysUser);
