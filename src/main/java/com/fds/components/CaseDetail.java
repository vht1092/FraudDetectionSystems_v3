...
public class CaseDetail extends CustomComponent implements CaseRegister {
	...
	public CaseDetail(final String caseno) {
		...
		ruleService = (EbankRuleService) helper.getBean("ruleService");
	...
	private void createForm() {
		...
		if (listCaseDetail.isEmpty()) {
			...
			final String sHtmlCaseDetail = 
					...
					+ "<tr><th>ECI Value:</th><td>"+ sEciVal +"</td></tr>" 
					+ "</table>";
			/* @formatter:on */
			...
			StringBuilder sHtmlHolderDetail = new StringBuilder("<table style='width:100%,text-align:left'>"
					...
					+ "<tr><th>Số điện thoại 2: </th><td>" + sOffTel1 + "<td></tr>"
			...
			/* ----- DANH SACH RULE ----- */
			final List<FdsEbankRule> ruleResult = ruleService.findByCaseNo(sCaseNo);
	...
	private Button.ClickListener eventClickBTDiscard() {
		return event -> {
			...
			try {
				...
				getUI().addWindow(createWindowComponent("Đóng case", new DiscardForm(this::closeWindow, this.sCaseno, this.sCardno, arCaseNo, this.sStatus)));
			} catch (Exception e) {
				Notification.show(ERROR_MESSAGE, Type.ERROR_MESSAGE);
