...
public class CaseDistribution extends CustomComponent implements ReloadAutoComponent, ReloadComponent {
	...
	private transient CaseEbankDetailGridComponent grid;
	private final transient VerticalLayout mainLayout = new VerticalLayout();
	private transient Page<FdsEbankCaseDetail> result;
	...
	private Page<FdsEbankCaseDetail> getData(Pageable page) {
		...
		result = ebankCaseDetailService.findAllBycheckNew(page);
		if (result.hasContent()) {
			tempCreTmsMax = result.getContent().stream().max(Comparator.comparing(FdsEbankCaseDetail::getTxnCreTms)).get().getTxnCreTms();
	...
	public void eventReloadAuto() {
		if (chboxRefreshGrid.getValue()) {
			grid.refreshData(getData(new PageRequest(result.getNumber(), SIZE_OF_PAGE, Sort.Direction.DESC, "txnCreTms")));
			// Refresh paging button
			refreshPaging();
		}
	}
