...
public class SystemStatistic extends VerticalLayout implements ReloadComponent {
	...
	private EbankCaseDetailService ebankCaseDetailService;
	...
	public SystemStatistic() {
		...
		lbTotalUnassignedCases.setValue("Tổng số case chưa xử lý: [" + ebankCaseDetailService.countAllNewestUserNotAssigned() + "]");
		lbTotalAssignedCases.setValue("Tổng số case đã xử lý: [" + ebankCaseDetailService.countAllClosedCase() + "]");
